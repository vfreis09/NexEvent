...
const EventList: React.FC<EventListProps> = ({
  ...
  const [cancelTarget, setCancelTarget] = useState<EventData | null>(null);

  const handleCancelClick = (event: EventData) => {
  ...
  const handleConfirmCancel = async () => {
    ...
    await cancelEvent(targetId, (data) => {
      onEventUpdate(data.event);
    });
  };

  const handleCloseCancelModal = () => setCancelTarget(null);
        ...
        >
          ...
          >
            ...
            <p className="mb-6 text-sm leading-relaxed text-muted-foreground">
              ...
              {cancelTarget.number_of_attendees > 0 && (
                <span className="mt-2 block font-semibold text-orange-600 dark:text-orange-400">
                  This will notify {cancelTarget.number_of_attendees} attendee
                  {cancelTarget.number_of_attendees !== 1 ? "s" : ""}.
                </span>
              )}
            </p>
